...
-(void)dowithDataWithDic:(NSDictionary *)dic
{
    if (dic) {
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    if (![receiveStr containsString:@"#EOF"]) {
        [receivedString appendString:receiveStr];
        [sock readDataWithTimeout:-1 tag:0];
        return;
    }else{
        NSString *temp = [receiveStr stringByReplacingOccurrencesOfString:@"#EOF" withString:@""];
        [receivedString appendString:temp];
    }
    NSString *dowithString = [receivedString copy];
    [receivedString deleteCharactersInRange:NSMakeRange(0, receivedString.length)];
    NSLog(@"dowith Data: %@",dowithString);
    NSDictionary *dic = [self objectWithJsonString:dowithString];
    if (dic == nil) {
        [sock readDataWithTimeout:-1 tag:0];
        return;
    }
    NSString *label = [dic objectForKey:@"label"];
    if ([label isEqualToString:@"http"]) {
        [self reqeustWithDic:dic];
    }else if([label isEqualToString:@"encrypt"]){
        [self dowithDataWithDic:dic];
    }else if([label isEqualToString:@"decrypt"]){
        [self dowithDataWithDic:dic];
    }else if ([label isEqualToString:@"heartbeat"]){
        [self writeDataToClient:dic];
    }
    [sock readDataWithTimeout:-1 tag:0];
}
